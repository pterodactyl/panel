...
const EditSubuserModal = ({ subuser }: Props) => {
  ...
  const editablePermissions = useDeepCompareMemo(() => {
    const cleaned = Object.keys(permissions).map((key) => Object.keys(permissions[key].keys).map((pkey) => `${key}.${pkey}`));
  ...
  return (
    ...
    >
      <Form>
        ...
        <div css={tw`my-6`}>
          <div css={tw`flex items-center mb-4 bg-neutral-600 p-2 rounded`}>
            <p css={tw`text-sm uppercase flex-1 ml-1 text-neutral-200`}>Select all permissions</p>

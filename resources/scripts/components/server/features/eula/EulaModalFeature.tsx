...
const EulaModalFeature = () => {
    ...
    return (
        <Modal visible={visible} onDismissed={() => setVisible(false)} closeOnBackground={false} showSpinnerOverlay={loading}>
            ...
            <h2 css={tw`text-2xl mb-4 text-neutral-100`}>{t('Eula Accept Title')}&reg; {t('Eula Accept Title And Desc')}</h2>
            <p css={tw`text-neutral-200`}>
                    {t('Eula Desc 1')} <strong>{t('Eula Accept Desc And Button')}</strong> {t('Eula Accept Desc 1')}&nbsp;
                <a
                    target={'_blank'}
                ...
                >
                        {t('Eula Accept Desc 2')}&reg; {t('Eula Accept Title And Desc')}
                </a>.
            </p>
